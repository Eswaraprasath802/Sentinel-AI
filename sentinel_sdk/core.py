...
class Sentinel:
    ...
    def __init__(
        ...
        # General
        app_name: str = "MyApp",
        silent: bool = False,   # default False so terminal alerts show
        on_fix=None,
    ):
    ...
    def attach(self, app):
        ...
        if framework == "flask":
            self._attach_flask(app)
        elif framework == "fastapi":
            self._attach_fastapi(app)
        elif framework == "django":
            self._attach_django(app)
        elif framework == "wsgi":
            return self._wrap_wsgi(app)
        else:
            pass  # global hook still active

        self.monitor.start()
        print(f"\033[92m[Sentinel] ✅ Attached to {framework} — monitoring active\033[0m")
        return app

    def watch(self):
        self.monitor.start()
        print("\033[92m[Sentinel] ✅ Watching — monitoring active\033[0m")
    ...
    def _attach_flask(self, app):
        ...
        def _after(response):
            ...
            if response.status_code >= 500 or ms > 5000:
                monitor.report_http_error(request.path, request.method, response.status_code, ms, "flask")
            return response

        @app.errorhandler(Exception)
        def _error(e):
            from flask import request, jsonify
            monitor.report_exception(e, traceback.format_exc(),
                                     {"path": request.path, "method": request.method}, "flask")
            return jsonify({"error": "Internal Server Error", "sentinel": "investigating"}), 500
                ...
                async def dispatch(self, request: Request, call_next):
                    ...
                    try:
                        ...
                        if resp.status_code >= 500 or ms > 5000:
                            monitor.report_http_error(request.url.path, request.method, resp.status_code, ms, "fastapi")
                        return resp
                    except Exception as e:
                        monitor.report_exception(e, traceback.format_exc(), {"path": request.url.path}, "fastapi")
